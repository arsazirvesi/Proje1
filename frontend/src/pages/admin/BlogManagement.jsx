  ...
  return (
    <div data-testid="blog-management-page">
      <div className="flex items-start justify-between mb-7">
        <div>
          <h1 className="font-heading text-summit-navy text-2xl sm:text-3xl">Blog Yönetimi</h1>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full admin-table">
            ...
            <tbody>
              {posts.length === 0 && <tr><td colSpan={5} className="text-center py-10 text-gray-500">Henüz blog yazısı yok</td></tr>}
              {posts.map(p => (
                <tr key={p.id} data-testid={`blog-row-${p.id}`}>
                  <td>
                    <div className="text-summit-navy text-sm font-medium">{p.title}</div>
                    <div className="text-gray-500 text-xs mt-0.5">/{p.slug}</div>
                  </td>
                  <td className="hidden sm:table-cell text-gray-500 text-sm">{p.author}</td>
                  <td>
                  ...
                  <td className="hidden md:table-cell text-gray-500 text-xs">{p.created_at?.slice(0,10)}</td>
                  <td>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/40">
          <div className="bg-summit-paper border border-gray-200 rounded-2xl p-6 w-full max-w-2xl shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-heading text-summit-navy text-lg">{editing ? "Yazıyı Düzenle" : "Yeni Blog Yazısı"}</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-gray-500 text-xs uppercase tracking-wider mb-2 block">Slug *</label>
                <input type="text" placeholder="yazı-slug" value={form.slug} onChange={e => setForm({...form, slug: e.target.value})}
                  className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-summit-navy text-sm focus:outline-none focus:border-summit-gold/50 font-mono" />
              </div>
              <div>
                <label className="text-gray-500 text-xs uppercase tracking-wider mb-2 block">Özet</label>
              ...
              <div>
                <label className="text-gray-500 text-xs uppercase tracking-wider mb-2 block">İçerik *</label>
                <textarea placeholder="Blog içeriği..." rows={8} value={form.content} onChange={e => setForm({...form, content: e.target.value})}
                  className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-summit-navy text-sm focus:outline-none focus:border-summit-gold/50 resize-none" />
              </div>
              <div>
                <label className="text-gray-500 text-xs uppercase tracking-wider mb-2 block">Kapak Görseli URL</label>
                <input type="url" placeholder="https://..." value={form.image_url} onChange={e => setForm({...form, image_url: e.target.value})}
                  className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-summit-navy text-sm focus:outline-none focus:border-summit-gold/50" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-gray-500 text-xs uppercase tracking-wider mb-2 block">Yazar</label>
                  <input type="text" placeholder="Admin" value={form.author} onChange={e => setForm({...form, author: e.target.value})}
                    className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-summit-navy text-sm focus:outline-none focus:border-summit-gold/50" />
                </div>
                <div>
                  <label className="text-gray-500 text-xs uppercase tracking-wider mb-2 block">Etiketler (virgülle)</label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-gray-600 text-sm">Yayınla (hemen görünür olsun)</span>
              </label>
