  ...
  return (
    <div data-testid="sponsor-management-page">
      <div className="flex items-start justify-between mb-7">
        <div>
          <h1 className="font-heading text-white text-2xl sm:text-3xl">Sponsor Yönetimi</h1>
          <p className="text-summit-text-muted text-sm mt-1">{sponsors.length} sponsor</p>
        </div>
      ...
      <div className="bg-summit-paper border border-white/8 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full admin-table">
            ...
            <tbody>
              {sponsors.length === 0 && <tr><td colSpan={5} className="text-center py-10 text-summit-text-muted">Sponsor bulunamadı</td></tr>}
              {sponsors.map(s => (
                <tr key={s.id} data-testid={`sponsor-row-${s.id}`}>
                  <td className="text-white font-medium">{s.name}</td>
                  <td>
                    <span className={`text-xs px-2 py-0.5 rounded font-medium ${s.tier === "main" ? "bg-summit-gold/15 text-summit-gold" : s.tier === "organization" ? "bg-blue-500/15 text-blue-400" : "bg-white/5 text-summit-text-muted"}`}>
                      {getTierLabel(s.tier)}
                    </span>
                  </td>
                  <td className="hidden sm:table-cell text-summit-text-muted text-xs">{s.website_url || "-"}</td>
                  <td className="text-summit-text-muted">{s.order}</td>
                  <td>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60">
          <div className="bg-summit-paper border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-heading text-white text-lg">{editing ? "Sponsor Düzenle" : "Yeni Sponsor"}</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-summit-text-muted text-xs uppercase tracking-wider mb-2 block">Tier</label>
