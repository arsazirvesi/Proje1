...
## Completed
- 2026-02-19: **Expert Portal — daha sakin palet + ayrı detay sayfası.** `ExpertGameList` sayfasında amber gradyan orbları + dot pattern kaldırıldı, neutral `bg-slate-50` zemin ve beyaz stat tile'larla göz yormayan bir görünüm. Drawer yerine yeni `/uzman/yatirim-oyunu/:id` route + `ExpertGameDetail` sayfası (tam sayfa portföy + yorumlar + Sunum Modu CTA). Karta tıklayınca `navigate(...)` ile ayrı sayfa açılıyor.
