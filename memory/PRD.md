...
## Completed
- 2026-02-13: **Fair Management admin — floor plan image upload.** `FairManagement.jsx` now lets admin upload PNG/JPG kroki directly (in addition to the PDF URL). Live preview thumbnail, "Görsel Yükle" button + URL fallback + "Görseli kaldır". Gallery section also gained a "Foto Yükle" button. Public `/fuar-alani` PDF download button now hidden when no PDF URL is set.
- 2026-02-13: **Fair registration no longer requires email verification.** `/api/register/guest` auto-verifies fair guests on submit, sends badge attachment via email immediately, pushes to Visitego. Summit still requires invite-code + email verification.
