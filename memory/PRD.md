...
## Completed
- 2026-05-23: **Seminer Kaydı Sistemi.** Seminer detay sayfasındaki "Kayıt Ol" butonu artık `/seminer-kaydi?slug=...&title=...` URL'sine yönlendiriyor; aynı VisitorRegisterPage formu seminer için yeniden kullanılıyor (davet kodu / zirve kapasite kontrolü atlanır, başlık seminer adından üretilir). Backend `POST /api/register/guest` artık `visit_type="seminar"` kabul ediyor — `seminar_slug` + `seminar_title` saklanır, Visitego push atlanır. Admin yan menüsüne **🎓 Seminer Kayıtları** linki + `/admin/seminer-kayitlari` route eklendi (GuestList `forcedVisitType="seminar"` ile). `GET /api/admin/guests?visit_type=seminar` filtresi de uygulandı. Test: 29/29 backend testi geçti (iteration_8).
- 2026-05-23: **Backend Modüler Refactor.** `/app/backend/server.py` 4200+ satırdan 576 satıra indirildi. Tüm route'lar `/app/backend/routes/` altına factory pattern ile çıkarıldı: `content.py`, `gallery_routes.py`, `registration.py`, `badge.py`, `admin_ops.py`, `crm.py`, `visitego_routes.py`, `investment.py`. Ortak yardımcılar: `email_service.py`, `utils.py`. SEO route sırası bug'ı düzeltildi (`/api/seo/robots.txt` & `/api/seo/sitemap.xml` artık 200 dönüyor). `render_badge_png` modül seviyesine taşındı (badge.py içinde) → registration.py ve crm.py'den import edilebilir. Test: 29/29 (%100, iteration_8).
- 2026-05-23: **Galeri Sistemi.** `/galeri` public sayfası + `/admin/galeri` yönetim paneli. 3 içerik tipi: Fotoğraf (R2 upload), Video (R2 upload, maks 300MB), YouTube (URL→thumbnail/embed otomatik). Public sayfa: tip/yıl filtresi, grid, lightbox. Admin: tip seçici, dosya yükleme, URL girişi, CRUD. Backend: `gallery_items` koleksiyonu + `/api/admin/uploads/video`.
- 2026-05-23: **Ana Sayfa Hero Section Yeniden Tasarımı.** Açık beyaz arka plan → koyu lacivert (#1A264F) + diagonal altın çizgi SVG + glassmorphic bileşenler. `WATCHPACK_POLLING=true` eklendi. Test: %100 (iteration_6).
